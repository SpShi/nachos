...
AddrSpace::AddrSpace(OpenFile *executable)
{
    ...
    //freeBitMap=new BitMap(NumPhysPages);
// how big is address space?
    ...
    printf("currentthread id :%d\taddr: ",spaceID);
    bitmap->Print();
// zero out the entire address space, to zero the unitialized data segment 
// and the stack segment
        char *tempMainMemory = new char[MemorySize];
...
AddrSpace::~AddrSpace()
{
   ...
   //printf("~addr currentThread: %d\t",spaceID);
   bitmap->Print();
   printf("page numbers: %d\n",numPages);
   for (int i = 0; i < numPages; i++)
   {
        //printf("pagetable: %d\n",pageTable[i].physicalPage);
