...
OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    // if ((numBytes <= 0) || (position >= fileLength))//第一个约束，append取到to的文件尾，直接return
	// return 0;				// check request
    if ((numBytes <= 0) || (position > fileLength)) // 修改后的第一个约束
        return -1;
    // if ((position + numBytes) > fileLength) // 第二个约束，超过文件长度后，numbytes改为0，不写
    //     numBytes = fileLength - position;
    // 修改后的第二个约束
    if ((position + numBytes) > fileLength) {
        //numBytes = fileLength - position;
        int increcmentBytes=(position+numBytes)-fileLength;
        BitMap *freeBitMap=fileSystem->getBitMap();
        //freeBitMap->Print();
        bool hdrRet;
        hdrRet=hdr->Allocate(freeBitMap,fileLength,increcmentBytes);
        if(!hdrRet)
            return -1;
