...
{ 
    ...
    //whether a new sector needed
    numBytes = fileSize;
    int offsetr=numBytes%SectorSize;
    int newSectorBytes=incrementBytes-(SectorSize-(offsetr+1));
    printf("offsetr:%d\tnewSectorBytes:%d\n",offsetr,newSectorBytes);
    //current sectors(last sector) are enough, don't need to allocate new sectors
    if(newSectorBytes<=0){
    ...
    printf("not enough! moreSectors: %d\n",moreSectors);
